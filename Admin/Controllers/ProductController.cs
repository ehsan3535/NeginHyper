...
namespace Client.Controllers
{
	public class ProductController : Controller
	{
		...
		public async Task<IActionResult> ReplaceForLocal(CancellationToken cancellationToken)
		{
			...
			{
				item.ImageCoverUrl = item.ImageCoverUrl.Replace("http://admin.NeginHyper.ir", "https://localhost:44382");
		...
		public async Task<IActionResult> ReplaceForServer(CancellationToken cancellationToken)
		{
			...
			foreach (var item in productList)
			{
				item.ImageCoverUrl = item.ImageCoverUrl.Replace("https://localhost:44382", "http://admin.NeginHyper.ir");
				productripo.Update(item);
			}
		...
		public async Task<IActionResult> Ir(CancellationToken cancellationToken)
		{
			...
			foreach (var item in productList)
			{
				if (item.ImageCoverUrl != null)
				{
					item.ImageCoverUrl = item.ImageCoverUrl.Replace("http://admin.NeginHyper.Com", "http://admin.NeginHyper.ir");
					productripo.Update(item);
				}
			}
			var Productimages = productImageripo.TableNoTracking.ToList();
			foreach (var item in Productimages)
			{
				item.ImageLink = item.ImageLink.Replace("http://admin.NeginHyper.Com", "http://admin.NeginHyper.ir");
				productImageripo.Update(item);
			}
			var categories = categoryripo.TableNoTracking.ToList();

			foreach (var item in categories)
			{
				if (item.ImageUrl != null)
				{
					item.ImageUrl = item.ImageUrl.Replace("http://admin.NeginHyper.Com", "http://admin.NeginHyper.ir");
					categoryripo.Update(item);
				}
			}
			return RedirectToAction("shop");
		}
		public async Task<IActionResult> Irsetting(CancellationToken cancellationToken)
		{
			var productList = settingrepo.TableNoTracking.ToList();
			foreach (var item in productList)
			{
				item.TopImageLink1 = item.TopImageLink1.Replace("http://admin.NeginHyper.Com", "http://admin.NeginHyper.ir");
				item.TopImageLink2 = item.TopImageLink2.Replace("http://admin.NeginHyper.Com", "http://admin.NeginHyper.ir");
				item.TopImageLink3 = item.TopImageLink3.Replace("http://admin.NeginHyper.Com", "http://admin.NeginHyper.ir");
				item.BottemLeftImageLink = item.BottemLeftImageLink.Replace("http://admin.NeginHyper.Com", "http://admin.NeginHyper.ir");
				item.BottemRightImageLink = item.BottemRightImageLink.Replace("http://admin.NeginHyper.Com", "http://admin.NeginHyper.ir");
				settingrepo.Update(item);
			}
			return RedirectToAction("shop");
		}
	}
